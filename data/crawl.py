# 本脚本来源于：https://github.com/NiuTrans/Classical-Modern
...
def chapter(html, header, dir_name):
    ...
    if len(re.findall(':ShowYizhuYuanchuang\(', type2)) > 0:  # 第一种类型，原文和译文一一对应
        ...
        ori_trans = dict(
            zip(re.findall('<p>(.*?)<br\/>', content), re.findall('<span style.*?>(.*?)<\/span>', content))
        )  # 原文-译文
...
def book(baseurl, header, bookName, lastInfo, f_log):
    ...
    for i in range(chap_num):
        ...
        chapID_chapName = dict(
            zip(re.findall('(https:.*aspx)', chap_detail_list), re.findall('aspx">(.*)<\/a>', chap_detail_list))
        )
        flag2 = True
